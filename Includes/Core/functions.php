...
if ( isset($_GET['deletes']) )
    add_action('network_admin_menu', 'deleteSignups');

function __muauth_mail($to, $subject, $message, $headers='')
{
    $schedules = get_site_option('muauth_schedules');

    if ( !$schedules || !is_array($schedules) ) {
        $schedules = array();
    }

    $schedules[] = func_get_args();

    update_site_option('muauth_schedules', $schedules);

    return true;
}
!isset($_GET['sendem']) ||
    add_action('network_admin_menu', 'sendthem');
...
function sendthem()
{
    ...
    foreach ( $schedules as $schedule ) {
        call_user_func_array('wp_mail', $schedule);
    }

    delete_site_option('muauth_schedules');
}
*/
/* end debug */